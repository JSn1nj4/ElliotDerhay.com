<div class='sidebar-track relative h-full w-6 overflow-x-clip'>
	<div
		class='relative h-full flex{{ $mirror ? ' -scale-x-100' : '' }} {{ $mirror ? 'translate-x-32' : '-translate-x-32' }} dark2:sm:translate-x-0 transition-transform navigating:transition-none'>
	...
	<div
		class='fixed top-0{{ $mirror ? ' -scale-x-100' : '' }} {{ $mirror ? 'translate-x-32' : '-translate-x-32' }} dark2:sm:translate-x-0 w-6 h-screen opacity-0 scroll:opacity-100 transition-all navigating:transition-none duration-700 ease-in-out'>
		<div
